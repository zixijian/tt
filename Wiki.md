...
要查看 `"simple"` 变量是否存在，您可以使用 `&{simple}`，此处括号不能省略，如果变量不存在，它将显示 0，如果变量存在，它将显示变量的索引。
...
要匹配 3-12 个字母：`{[A-Za-z]{3,12}}`。

要匹配任意个纯字母：`{[[:alpha:]]+}` 或 `{[A-Za-z]+}`。
...
行号 row 是可选且实用的参数，有助于非标准屏幕拆分模式的显示。  

正行号从顶部绘制 #row 行，而负行号从底部绘制 #row 行。

没有参数时 `#prompt` 将写入默认的分割行，即输入行上面的一行，通常位于 `row -2`。
...
[showme](#showme) 和 [echo](#echo) 命令也可以使用 row、col 参数，因此使用 `#show` 和 `#echo` 命令在也可以在分割窗口上显示文本。
...
如果要匹配 0 到任意个字母，请使用 `{[A-Za-z]{0,}}`，以此类推。
...
示例：
#regex 0324killmaster {^{[[:digit:]]+}{[[:alpha:]]+}$} {#show [&0] [&1] [&2]}
[0324killmaster] [0324] [killmaster]
...
按下 “Tab” 键时，TinTin 将在您的自动补全列表中搜索最佳匹配，对于长命令很有用。
...
正常一些不会输出消息的事件会产生大量冗杂信息，除非您在同一类别设置中已经有了一个事件。
```
...
如果你想变得有创意，还相当于：`#var hp[self] 34;#var {hp} {$hp} {{target}{46}}`。
